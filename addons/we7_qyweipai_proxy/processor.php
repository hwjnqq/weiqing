...
class We7_qyweipai_proxyModuleProcessor extends WeModuleProcessor {
	public function respond() {
		$reply = pdo_fetch("SELECT * FROM ".tablename('we7_qyweipai_proxy_reply')." WHERE rid = :rid", array(':rid' => $this->rule));
		$response = $this->procRemote(array('apiurl' => $reply['proxyurl'], 'token' => $reply['proxytoken']));
		if (strexists($response['Content'], '欢迎进入微拍活动')) {
			$this->beginContext();
		}
		if (strexists($response['Content'], '参与活动成功')) {
			$this->endContext();
			return $response;
		}
