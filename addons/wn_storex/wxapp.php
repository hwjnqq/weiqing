...
class Wn_storexModuleWxapp extends WeModuleWxapp {
	...
	function actions($ac) {
		$actions = array(
			...
			'homePageList' => array('do' => 'homepageapp', 'op' => 'display'),
			'homePageNotice' => array('do' => 'homepageapp', 'op' => 'notice'),
...
// 					mc_notice_recharge($order['openid'], $order['uid'], $total_fee, '', $remark);
			...
			if (!empty($storex_bases['phones'])) {
				...
				foreach ($storex_bases['phones'] as $phone) {
					cloud_prepare();
					$body = 'df';
					$body = '用户' . $order['name'] . ',电话:' . $order['mobile'] . '于' . date('m月d日H:i') . '成功支付订单' . $order['ordersn']
