...
function calculate_carriage ($goods_info, $insert) {
	if ($insert['mode_distribute'] == 2) {
		if (!empty($goods_info['carriage_set'])) {
			$carriage_set = iunserializer($goods_info['carriage_set']);
			if ($insert['sum_price'] < $carriage_set['full_free']) {
				$insert['sum_price'] += $carriage_set['carriage'];
			}
		}
	}
	return $insert;
