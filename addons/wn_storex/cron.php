...
class Wn_storexModuleCron extends WeModuleCron {
	public function doCronGroup() {
		...
		$group_activity = pdo_get('storex_plugin_group_activity', array('id' => $id));
		//活动下的商品
		$activity_goods = pdo_get('storex_plugin_activity_goods', array('group_activity' => $group_activity['id']));
		//开始的团   over 1完成2未完成3已退款
		$group = pdo_getall('storex_plugin_group', array('activity_goodsid' => $activity_goods['id'], 'over' => 2), array(), 'id');
		$group_ids = array_keys($group);
		if (!empty($group)) {
			$order = pdo_getall('storex_order', array('group_id' => $group_ids, 'paystatus' => 1), array(), 'id');
			if (!empty($order)) {
