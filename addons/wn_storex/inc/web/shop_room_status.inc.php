...
if ($op == 'getDate') {
	...
	$list = pdo_getall('storex_room', array('hotelid' => $storeid, 'weid' => $_W['uniacid'], 'is_house' => 1));
...
if ($op == 'submitPrice') {
	...
	$date = $_GPC['date'];
	$roomprice = getRoomPrice($storeid, $roomid, $date);
	if ($pricetype == 'num') {
	...
	die(json_encode(array('result' => 1, 'hotelid' => $storeid, 'roomid' => $roomid, 'pricetype' => $pricetype, 'price' => $price)));
}
...
if ($op == 'updatelot') {
	...
	$rooms = pdo_getall('storex_room', array('hotelid' => $storeid, 'is_house' => 1));
	include $this->template('store/shop_room_statuslot');
	exit();
}
...
if ($op == 'updatelot_create') {
	...
	ob_start();
	include $this->template('store/shop_room_statuslotlist');
	$data['result'] = 1;
	$data['code'] = ob_get_contents();
	ob_clean();
	die(json_encode($data));
	...
	foreach ($rooms_arr as $v) {
		for ($time = $start; $time <= $end; $time+=86400) {
			...
			if (in_array($week, $days_arr)) {
				$roomprice = getRoomPrice($storeid, $v, date('Y-m-d', $time));
...
//当月最后一天
$endtime = strtotime(date('Y-m-d', strtotime("$firstday +1 month -1 day")));
include $this->template('store/shop_room_status');