...
if (!pdo_fieldexists('shake_member', 'status')) {
	pdo_query("ALTER TABLE " . tablename('shake_member') . " ADD `status` tinyint(4) NOT NULL DEFAULT '0' AFTER `lastupdate`;");
}
...
if (!pdo_fieldexists('shake_reply', 'uniacid')) {
	pdo_query('ALTER TABLE ' . tablename('shake_reply') . ' ADD `uniacid` INT(10) UNSIGNED NOT NULL AFTER `rid`;');
}