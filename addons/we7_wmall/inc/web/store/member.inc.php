...
if($op == 'sync') {
	if($_W['isajax']) {
		$uid = intval($_GPC['uid']);
		$update = array();
		$update['success_num'] = intval(pdo_fetchcolumn('select count(*) from ' . tablename('tiny_wmall_store_members') . ' where uniacid = :uniacid and sid = :sid  and is_pay = 1 and status = 5', array('uniacid' => $_W['uniacid'], 'sid' => $sid)));
		$update['success_price'] = floatval(pdo_fetchcolumn('select sum(final_fee) from ' . tablename('tiny_wmall_store_members') . ' where uniacid = :uniacid and sid = :sid  and is_pay = 1 and status = 5', array('uniacid' => $_W['uniacid'], 'sid' => $sid)));
		$update['cancel_num'] = intval(pdo_fetchcolumn('select sum(final_fee) from ' . tablename('tiny_wmall_store_members') . ' where uniacid = :uniacid and sid = :sid  and status = 6', array('uniacid' => $_W['uniacid'], 'sid' => $sid)));
		$update['cancel_price'] = floatval(pdo_fetchcolumn('select sum(final_fee) from ' . tablename('tiny_wmall_store_members') . ' where uniacid = :uniacid and sid = :sid  and status = 6', array('uniacid' => $_W['uniacid'], 'sid' => $sid)));
