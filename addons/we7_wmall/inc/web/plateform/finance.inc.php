...
global $_W, $_GPC;
$_W['page']['title'] = '财务统计-' . $_W['wmall']['module']['name'];
mload()->model('finance');
$do = 'finance';
...
if($op == 'day_order_price') {
	...
	if($_W['isajax'] && $_W['ispost']) {
		$datasets = array(
			'wechat' => array('name' => '微信支付', 'value' => 10),
		...
		$condition = 'WHERE uniacid = :uniacid AND sid = :sid and status = 5 and is_pay = 1 and addtime >= :starttime and addtime <= :endtime';
		$params = array(':uniacid' => $_W['uniacid'], ':sid' => $sid, ':starttime' => $start, 'endtime' => $end);
		$sid = intval($_GPC['sid']);
		if($sid > 0) {
			$condition .= ' and sid = :sid';
			$params[':sid'] = $sid;
		}
		$data = pdo_fetchall("SELECT id, pay_type FROM " . tablename('tiny_wmall_order') . $condition, $params);
		foreach($data as $da) {
...
if($op == 'day_order_num') {
	...
	if($_W['isajax'] && $_W['ispost']) {
		...
		$data = pdo_fetchall("SELECT id FROM " . tablename('tiny_wmall_order') . $condition, $params);
		foreach($data as $da) {
			if(in_array($da['pay_type'], array_keys($datasets))) {
