...
if($op == 'list') {
	...
	if(!empty($list)) {
		foreach($list as &$da) {
			$da['store_ids'] = explode(',', $da['store_ids']);
		}
	}
}
...
if($op == 'post') {
	...
	if(checksubmit()) {
		...
		$permission = 'tiny_wmall_menu_store';
		$permission_exist = pdo_get('users_permission', array('uniacid' => $_W['uniacid'], 'uid' => $account['uid'], 'type' => 'tiny_wmall'));
		if(empty($permission_exist)) {
			pdo_insert('users_permission', array('uniacid' => $_W['uniacid'], 'uid' => $account['uid'], 'type' => 'tiny_wmall', 'permission' => $permission));
		} else {
			pdo_update('users_permission', array('permission' => $permission), array('uniacid' => $_W['uniacid'], 'uid' => $account['uid'], 'type' => 'tiny_wmall'));
		}
		...
		if(!empty($_GPC['store_ids'])) {
			foreach($_GPC['store_ids'] as $key => $val) {
				$val = intval($val);
				if(!$val) {
					continue;
				}
		...
		$data = array(
			...
			'lastip' => CLIENT_IP,
			'status' => 2,
			'store_nums' => intval($_GPC['store_nums']),
