...
if($op == 'list') {
	$title = '订单列表';
	$orders = pdo_fetchall('select a.id as aid, a.*, b.title, b.logo, b.pay_time_limit from ' . tablename('tiny_wmall_order') . ' as a left join ' . tablename('tiny_wmall_store') . ' as b on a.sid = b.id where a.uniacid = :uniacid and a.uid = :uid order by a.id desc limit 15', array(':uniacid' => $_W['uniacid'], ':uid' => $_W['member']['uid']), 'aid');
	$min = 0;
	if(!empty($orders)) {
...
if($op == 'more') {
	$orders = pdo_fetchall('select a.id as aid, a.*, b.title, b.logo, b.pay_time_limit from ' . tablename('tiny_wmall_order') . ' as a left join ' . tablename('tiny_wmall_store') . ' as b on a.sid = b.id where a.uniacid = :uniacid and a.uid = :uid order by a.id desc limit 10', array(':uniacid' => $_W['uniacid'], ':uid' => $_W['member']['uid']), 'aid');
	$min = 0;
	if(!empty($orders)) {
		$order_status = order_status();
		foreach($orders as &$order) {
			$order['goods'] = pdo_get('tiny_wmall_order_stat', array('oid' => $order['id']), array('goods_title'));
...
if($op == 'end') {
	...
	pdo_update('tiny_wmall_order', array('status' => '5'), array('uniacid' => $_W['uniacid'], 'id' => $id));
	order_insert_status_log($id, $order['sid'], 5);
	message(error(0, ''), '', 'ajax');
}
...
if($op == 'detail') {
	...
	$store = store_fetch($order['sid'], array('title', 'telephone', 'pack_price', 'logo', 'delivery_price'));
...
if($op == 'comment') {
	...
	if(!$_W['ispost']) {
		...
		foreach($goods as $good) {
			...
			pdo_query('update ' . tablename('tiny_wmall_goods') . $update . ' where id = :id', array('id' => $good['id']));
		}
