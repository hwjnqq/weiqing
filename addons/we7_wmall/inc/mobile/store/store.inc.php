...
if($op == 'comment') {
	...
	$comments = pdo_fetchall('select a.id as aid, a.*, b.title from ' . tablename('tiny_wmall_order_comment') . ' as a left join ' . tablename('tiny_wmall_store') . ' as b on a.sid = b.id ' . $condition . ' order by a.id desc limit 3', $params, 'aid');
	$min = 0;
	if(!empty($comments)) {
		foreach ($comments as &$row) {
			...
			$row['avatar'] = tomedia($row['avatar']) ? tomedia($row['avatar']) : '../addons/we7_wmall/resource/app/img/noavatar_middle.gif';
		}
