...
class We7_communityModuleSite extends WeModuleSite {

    public function getMenuTiles() {
        global $_W, $_GPC;
        $menus = array();
        $list = pdo_fetchall("SELECT * FROM " . tablename(community_region) . " WHERE weid=:weid ", array(':weid' => $_W['uniacid']));
        if (!empty($list)) {
            foreach ($list as $row) {
    ...
    private function getServiceCategory($name) {
        global $_W;
        $category = pdo_fetch("SELECT * FROM " . tablename(community_servicecategory) . " WHERE weid=:weid AND name=:name ", array(':weid' => $_W['uniacid'], ':name' => $name));
        return $category;
    }
    ...
    private function getServiceCategories($parentid) {
        global $_W;
        $categories = pdo_fetchall("SELECT * FROM " . tablename(community_servicecategory) . " WHERE weid=:weid AND parentid=:parentid ORDER BY displayorder ASC", array(':weid' => $_W['uniacid'], ':parentid' => $parentid), 'id');
        return $categories;
    }
    ...
    public function doWebHomemaking() {
        ...
        if ($operation == 'display') {
            ...
            $starttime = empty($_GPC['datelimit']['start']) ? strtotime('-1 month') : strtotime($_GPC['datelimit']['start']);
            $endtime = empty($_GPC['datelimit']['end']) ? TIMESTAMP : strtotime($_GPC['datelimit']['end']) + 86399;
            ...
            if (!empty($list)) {
                ...
                $members = pdo_fetchall("SELECT realname, mobile, openid FROM " . tablename('community_member') . " WHERE openid IN ('" . implode("','", $members) . "') AND weid = '{$_W['uniacid']}'", array(), 'openid');
            }
        } elseif ($operation == 'delete') {

            pdo_delete(community_service, array('id' => intval($_GPC['id']), 'regionid' => $regionid, 'weid' => $_W['uniacid']));
            message('删除成功.', referer(), 'success');
        } elseif ($operation == 'post') {

            $id = intval($_GPC['id']);
            $item = pdo_fetch("SELECT * FROM " . tablename(community_service) . " WHERE id=:id AND regionid=:regionid AND weid=:weid", array(':id' => $id, ':weid' => $_W['uniacid'], 'regionid' => $regionid));
            if (empty($item)) {
                message('抱歉，查看的服务需求不存在或是已经被删除！');
            }
            $member = pdo_fetch("SELECT * FROM " . tablename(community_member) . " WHERE weid=:weid AND openid=:openid", array(':weid' => $_W['uniacid'], ':openid' => $item['openid']));
            if (checksubmit('submit')) {
                ...
                pdo_update('community_service', $data, array('id' => $id, 'weid' => $_W['uniacid'], 'regionid' => $regionid));
    ...
    public function doMobileHomemaking() {
        ...
        if ($op == 'display') {
            ...
            $pindex = max(1, intval($_GPC['page']));
            $psize = 50;
            $paras = array(':weid' => $_W['uniacid'], ':openid' => $_W['fans']['from_user'], ':servicecategory' => $pcate['id']);
            $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename(community_service) . " WHERE `weid`=:weid AND `openid` = :openid ", array(':weid' => $_W['uniacid'], ':openid' => $_W['fans']['from_user']));
            ...
            if (checksubmit('submit')) {
                ...
                if (empty($id)) {
                    ...
                    pdo_update('community_service', $data, array('id' => $id, 'weid' => $_W['uniacid'], 'openid' => $member['openid']));
                    $msg = '信息编辑成功。';
                }
                ...
                if (!empty($id)) {
                    ...
                    $paras = array(':weid' => $_W['uniacid'], ':openid' => $_W['fans']['from_user'], ':id' => $id);
                    $item = pdo_fetch($sql, $paras);
                }
            }
        } elseif ($op == 'delete') {

            pdo_delete(community_service, array('id' => intval($_GPC['id']), 'weid' => $_W['uniacid'], 'openid' => $member['openid']));
    ...
    public function doWebHouselease() {
        ...
        if ($operation == 'display') {
            ...
            pdo_delete(community_service, array('id' => intval($_GPC['id']), 'weid' => $_W['uniacid'], 'regionid' => $regionid));
            message('删除成功.', referer(), $this->createWebUrl('houselease'), 'success');
        } elseif ($operation == 'post') {

            $id = intval($_GPC['id']);
            $item = pdo_fetch("SELECT * FROM " . tablename(community_service) . " WHERE id=:id AND weid=:weid AND regionid=:regionid", array(':id' => $id, ':weid' => $_W['uniacid'], ':regionid' => $regionid));
            if (empty($item)) {
    ...
    public function doWebServiceCategory() {
        ...
        if ($operation == 'display') {
            ...
            $children = array();
            $category = pdo_fetchall("SELECT * FROM " . tablename('community_servicecategory') . " WHERE weid = '{$_W['uniacid']}' ORDER BY parentid ASC, displayorder ASC");
    ...
    private function getAds($regionid) {
        ...
        if (!empty($ads)) {
            ...
            $sql = "SELECT * FROM " . tablename('community_advertisement') . " WHERE `starttime` <= '{$_W['timestamp']}' AND `endtime` > '{$_W['timestamp']}' AND `status` = '1' AND `weid` ='{$_W['uniacid']}' AND `id` in ('" . implode("','", $adids) . "')";
            unset($ads);
    ...
    public function doWebAdvertisement() {
        ...
        if ($operation == 'display') {
            ...
            $regions = pdo_fetchall("SELECT * FROM " . tablename('community_region') . " WHERE weid=:weid", array(':weid' => $_W['uniacid']), 'id');
            foreach ($list as &$ad) {
            ...
            if (checksubmit()) {
                ...
                if (empty($id)) {
                    ...
                    if (is_array($region) && count($region) > 0) {
                        foreach ($region as $regionid) {
                            pdo_insert('community_admap', array('weid' => $_W['uniacid'], 'adid' => $id, 'regionid' => intval($regionid)));
                        }
                    }
                    ...
                    pdo_update('community_advertisement', $item, array('id' => $id, 'weid' => $_W['uniacid']));
                    pdo_delete('community_admap', array('weid' => $_W['uniacid'], 'adid' => $id));
                ...
                if (empty($item)) {
                    ...
                    $regionids = pdo_fetchall("SELECT regionid FROM " . tablename('community_admap') . " WHERE adid=:adid AND weid=:weid", array(':adid' => $item['id'], ':weid' => $_W['uniacid']), 'regionid');
            ...
            $id = intval($_GPC['id']);
            if (!empty($id)) {
                pdo_delete('community_advertisement', array('id' => $id, 'weid' => $_W['uniacid']));
                pdo_delete('community_admap', array('adid' => $id, 'weid' => $_W['uniacid']));
            }
    ...
    public function doWebAnnouncement() {
        ...
        if ($operation == 'display') {
            ...
            $id = intval($_GPC['id']);
            if (!empty($id)) {
                pdo_delete('community_announcement', array('id' => $id, 'weid' => $_W['uniacid'], 'regionid' => $regionid));
            }
            message('删除成功.', referer(), 'success');
        }
    ...
    public function doMobileAnnouncement() {
        ...
        if ($op == 'display') {
            ...
            $id = intval($_GPC['id']);
            if (!empty($id)) {
                $item = pdo_fetch("SELECT * FROM " . tablename('community_announcement') . " WHERE `id`=:id AND `weid`=:weid AND `regionid`=:regionid", array(':id' => $id, ':weid' => $_W['uniacid'], ':regionid' => $member['regionid']));
            }
            if (empty($item)) {
    ...
    public function doWebRepair() {
        ...
        if ($operation == 'display') {
            ...
            if (!empty($list)) {
                ...
                $members = pdo_fetchall("SELECT realname, mobile, openid FROM " . tablename('community_member') . " WHERE `openid` IN ('" . implode("','", $members) . "') AND `weid` = :weid AND `regionid`=:regionid", array(':weid' => $_W['uniacid'], ':regionid' => $regionid), 'openid');
            }
        } elseif ($operation == 'post') {

            $id = intval($_GPC['id']);
            $item = pdo_fetch("SELECT * FROM " . tablename('community_report') . " WHERE `id`=:id AND `type` = '1' AND `weid`=:weid AND `regionid`=:regionid", array(':id' => $id, ':weid' => $_W['uniacid'], ':regionid' => $regionid));
            if (empty($item)) {
            ...
            if (checksubmit('submit')) {
                ...
                //管理员可以更改报修复申请
                pdo_update('community_report', $data, array('id' => $id, 'weid' => $_W['uniacid'], 'regionid' => $regionid));
                ...
                if (!empty($_GPC['reply'])) {
                    ...
                    pdo_update('community_report', array('newmsg' => 0), array('id' => $id, 'weid' => $_W['uniacid'], 'regionid' => $regionid));
                }
            ...
            $id = intval($_GPC['id']);
            $item = pdo_fetch("SELECT * FROM " . tablename('community_report') . " WHERE `id`=:id AND `weid`=:weid AND `regionid`=:regionid", array(':id' => $id, ':weid' => $_W['uniacid'], ':regionid' => $regionid));
            if (empty($item)) {
                message('抱歉，查看的报修申请不存在或是已经被删除！');
            }
            pdo_delete('community_report', array('id' => $id, 'weid' => $_W['uniacid'], 'regionid' => $regionid));
            pdo_delete('community_reply', array('reportid' => $id, 'weid' => $_W['uniacid'], 'regionid' => $regionid));
    ...
    public function doWebReport() {
        ...
        if ($operation == 'display') {
            ...
            pdo_delete('community_report', array('id' => intval($_GPC['id']), 'weid' => $_W['uniacid'], 'regionid' => $_GPC['regionid']));
            pdo_delete('community_reply', array('reportid' => intval($_GPC['id'])));
            message('删除成功.', referer(), 'success');
        } elseif ($operation == 'post') {

            $id = intval($_GPC['id']);
            $item = pdo_fetch("SELECT * FROM " . tablename('community_report') . " WHERE `id`=:id AND `weid`=:weid ", array(':id' => $id, ':weid' => $_W['uniacid']));
            if (empty($item)) {
            ...
            if (checksubmit()) {
                ...
                pdo_update('community_report', $data, array('id' => intval($_GPC['id']), 'weid' => $_W['uniacid'], 'regionid' => $_GPC['regionid']));
    ...
    public function doMobileReport() {
        ...
        if ($op == 'display') {
            $pindex = max(1, intval($_GPC['page']));
            $psize = 50;
            $paras = array(':weid' => $_W['uniacid'], ':openid' => $_W['fans']['from_user']);
    ...
    public function doWebPhone() {
        ...
        if ($op == 'delete') {
            $id = intval($_GPC['id']);
            if (!empty($id)) {
                pdo_delete('community_phone', array('id' => $id, 'weid' => $_W['uniacid'], 'regionid' => $regionid));
            }
            ...
            if (checksubmit('submit')) {
                ...
                foreach ($phones as $key => $value) {
                    ...
                    if ($id == 0) {
                        ...
                        pdo_update('community_phone', $data, array('id' => $id, 'weid' => $_W['uniacid'], 'regionid' => $regionid));
                    }
                }
        ...
        $paras = array(':weid' => $_W['uniacid'], ':regionid' => $regionid);
    ...
    public function doMobilePhone() {
        ...
        $sql = "SELECT * FROM " . tablename('community_phone') . " WHERE `weid`=:weid AND `regionid`=:regionid";
        $paras = array(':weid' => $_W['uniacid'], ':regionid' => $member['regionid']);
        $phones = pdo_fetchall($sql, $paras);
        include $this->template('phone');
    }
    ...
    public function doWebRegion() {
        ...
        if ($op == 'delete') {
            ...
            //获取待办的报修申请
            $repaircount = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('community_report') . " WHERE type = '1' AND weid = '{$_W['uniacid']}' AND regionid = '{$id}' AND status = '0'");
            //获取待办的投诉申请
            $reportcount = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('community_report') . " WHERE type = '2' AND weid = '{$_W['uniacid']}' AND regionid = '{$id}' AND status = '0'");
            //待回复事项
            $reply = pdo_fetchall("SELECT * FROM " . tablename('community_report') . " WHERE newmsg = '1' AND weid = '{$_W['uniacid']}'", array(), 'id');
            if (!empty($reply)) {
    ...
    public function doWebMember() {
        ...
        if ($operation == 'display') {
            ...
            if (checksubmit('auditall')) {
                ...
                pdo_query($sql, array(':weid' => $_W['uniacid'], ':regionid' => $regionid));
            ...
            $needAudit = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('community_member') . " WHERE `weid`=:weid AND `regionid`=:regionid AND `status`=0", $paras = array(':weid' => $_W['uniacid'], ':regionid' => $regionid));
        } elseif ($operation == 'delete') {
            ...
            if (checksubmit('submit')) {
                ...
                $paras = array('id' => intval($_GPC['id']), 'weid' => $_W['uniacid']);
            ...
            $status = intval($_GPC['status']);
            $id = intval($_GPC['id']);
            pdo_update('community_member', array('status' => $status), array('id' => $id, 'weid' => $_W['uniacid']));
    ...
    public function doWebManager() {
        ...
        $memberids = array();
        $sql = 'SELECT * FROM ' . tablename('wechats_members') . " WHERE weid={$_W['uniacid']}";
        ...
        if ($op == 'display') {
            ...
            if (checksubmit()) {
                ...
                pdo_delete(modules_menus, array('acid' => $_W['uniacid'], 'memberid' => $memberid, 'module' => 'community'));
            ...
            $mymenus = pdo_fetchall($sql, array(':memberid' => $member['uid'], ':acid' => $_W['uniacid']));
    ...
    public function doMobileRegister() {
        ...
        $verify = pdo_fetch("SELECT * FROM " . tablename('community_verifycode') . " WHERE weid = '{$_W['uniacid']}' AND openid = :openid", array(':openid' => $_W['fans']['from_user']));
        ...
        if (checksubmit('submit')) {
            ...
            if ($step == '1') {
                $verify = pdo_fetch("SELECT * FROM " . tablename('community_verifycode') . " WHERE weid = '{$_W['uniacid']}' AND openid = :openid AND mobile = :mobile", array(':openid' => $_W['fans']['from_user'], ':mobile' => $data['mobile']));
                ...
                $mobileexists = pdo_fetch("SELECT mobile FROM " . tablename('community_member') . " WHERE mobile = :mobile AND weid = '{$_W['uniacid']}'", array(':mobile' => $data['mobile']));
    ...
    public function doMobileVerifyCode() {
        ...
        $member = pdo_fetch("SELECT id, mobile FROM " . tablename('community_member') . " WHERE mobile = :mobile AND weid = '{$_W['uniacid']}'", array(':mobile' => $mobile));
        if (!empty($member['id'])) {
        ...
        $verify = pdo_fetch("SELECT * FROM " . tablename('community_verifycode') . " WHERE openid = :openid AND mobile = :mobile AND weid = '{$_W['uniacid']}'", array(':openid' => $_W['fans']['from_user'], ':mobile' => $mobile));
        if ($verify['total'] >= 5) {
    ...
    private function getRegions() {
        ...
        $regions = pdo_fetchall($sql, array(':weid' => $_W['uniacid']), 'id');
        return $regions;
    }
    ...
    private function getRegion($id) {
        ...
        $region = pdo_fetch($sql, array(':id' => $id, ':weid' => $_W['uniacid']));
        return $region;
    }
    ...
    private function getRegionIdsByUid($uid = 0) {
        ...
        $regions = pdo_fetchcolumn("SELECT regions FROM " . tablename('community_manager') . " WHERE `weid`=:weid AND `uid`=:uid", array(':weid' => $_W['uniacid'], ':uid' => $_W['uid']));
        if (empty($regions)) {
            return array();
        }
    ...
    public function doWebExpressCompany() {
        ...
        if ($operation == 'display') {
            ...
            $list = pdo_fetchall('SELECT * FROM ' . tablename(community_express_company) . " WHERE `weid`=:weid", array(':weid' => $_W['uniacid']));
        } elseif ($operation == 'delete') {

            $id = intval($_GPC['id']);
            $express = pdo_fetch('SELECT * FROM ' . tablename(community_express_company) . " WHERE weid=:weid AND id=:id", array(':weid' => $_W['uniacid'], ':id' => $id));
            if (empty($express)) {
                message('快递未找到或已删除.', referer(), 'success');
            }
            $count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename(community_express_fee) . " WHERE weid=:weid AND code=:code", array(':weid' => $_W['uniacid'], ':code' => $express['code']));
            if ($count > 0) {
                message('无法删除快递,快递费用未删除.');
            }
            pdo_delete(community_express_company, array('id' => intval($_GPC['id']), 'weid' => $_W['uniacid']));
            message('删除成功.', referer(), 'success');
        }
    ...
    public function doWebExpressFee() {
        ...
        if ($operation == 'display') {
            ...
            if (checksubmit()) {
                ...
                $list = pdo_fetchall('SELECT * FROM ' . tablename(community_express_fee) . ' WHERE weid=:weid AND code=:code', array(':weid' => $_W['uniacid'], ':code' => $express['code']));
            }
        } elseif ($operation == 'delete') {
            $id = intval($_GPC['id']);
            pdo_delete(community_express_fee, array('id' => intval($_GPC['id']), 'weid' => $_W['uniacid']));
            message('删除成功.', referer(), 'success');
        }
    ...
    public function doWebExpress() {
        ...
        if ($operation == 'display') {
            ...
            $starttime = empty($_GPC['datelimit']['start']) ? strtotime('-1 month') : strtotime($_GPC['start']);
            $endtime = empty($_GPC['datelimit']['end']) ? TIMESTAMP : strtotime($_GPC['end']) + 86399;
            ...
            pdo_delete(community_express_order, array('id' => intval($_GPC['id']), 'weid' => $_W['uniacid']));
            message('删除成功.', referer(), 'success');
        } elseif ($operation == 'post') {
            ...
            if (checksubmit('submit')) {
                ...
                $id = intval($_GPC['id']);
                $item = pdo_fetch('SELECT * FROM ' . tablename(community_express_order) . " WHERE id=:id AND weid=:weid AND regionid=:regionid", array(':id' => $id, ':weid' => $_W['uniacid'], ':regionid' => $regionid));
                if (empty($item)) {
                    message('未找到指定快递单', $this->createWebUrl('express', array('op' => 'display', 'regionid' => $regionid, 'weid' => $_W['uniacid'])));
                } else {
                    ...
                    pdo_update(community_express_order, $data, array('id' => $id, 'weid' => $_W['uniacid'], 'regionid' => $regionid));
                    message('更新成功！', $this->createWebUrl('express', array('op' => 'display', 'regionid' => $regionid, 'weid' => $_W['uniacid'])), 'success');
                }
            } else {

                $id = intval($_GPC['id']);
                if (!empty($id)) {
                    $item = pdo_fetch('SELECT * FROM ' . tablename(community_express_order) . " WHERE id=:id AND weid=:weid", array(':id' => $id, ':weid' => $_W['uniacid']));
                }
    ...
    public function doMobileExpress() {
        ...
        if ($op == 'display') {
            ...
            pdo_delete(community_express_order, array('id' => intval($_GPC['id']), 'weid' => $_W['uniacid'], 'openid' => $_W['fans']['from_user']));
            message('删除成功.', referer(), 'success');
        } elseif ($op == 'post') {
            ...
            if (checksubmit('submit')) {
                ...
                $id = intval($_GPC['id']);
                $item = pdo_fetch('SELECT * FROM ' . tablename(community_express_order) . " WHERE id=:id AND weid=:weid AND openid=:openid", array(':id' => $id, ':weid' => $_W['uniacid'], 'openid' => $_W['fans']['from_user']));
                $member = $this->getMember();
                if (empty($item)) {
                    ...
                    pdo_update(community_express_order, $data, array('id' => $id, 'weid' => $_W['uniacid']));
    ...
    private function getExpressCompanys() {
        global $_W, $_GPC;
        $coms = pdo_fetchall('SELECT * FROM ' . tablename(community_express_company) . " WHERE weid=:weid", array(':weid' => $_W['uniacid']), 'code');
        return $coms;
    }
    ...
    public function getHomeTiles() {
        ...
        $sql = 'SELECT `id`, `title` FROM ' . tablename('community_region') . ' WHERE `weid` = :weid';
        $replies = pdo_fetchall($sql, array(':weid' => $_W['uniacid']));
        if (!empty($replies)) {
            foreach ($replies as $reply) {
                $urls[] = array('title' => $reply['title'], 'url' => $this->createMobileUrl('home', array('id' => $reply['id'])));
            }
        }
