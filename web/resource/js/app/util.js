...
//			'autoFloatEnabled': false
		...
		debugger;
		debugger;
		UE.registerUI('myinsertvoice', function(editor, uiName) {
			debugger;
			editor.registerCommand(uiName, {
				execCommand: function() {
					uploader.show(function(voice) {
						if(!video) {
							return;
						} else {
							var videoType = video.isRemote ? 'iframe' : 'video';
					...
					}, { fileSizeLimit: options.audio_limit * 1024, type: 'voice', allowUploadVideo: options.allow_upload_video, netWorkVideo : true });
				}
			});
			var btn = new UE.ui.Button({
				name: '插入语音',
